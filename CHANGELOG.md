...
+ [新增] 画布「连接」页在未连上本地 Agent 时显示一键安装入口：复制命令在 PowerShell 执行一次，本地 Agent 就会在后台自动运行（无命令行窗口、开机自启），不再需要每次手动启动。
...
+ [调整] Canvas Agent 内置 Codex 升级到 0.157.0，Agent 面板的模型列表同步到当前版本目录，可以选择 GPT-6 系列模型。
