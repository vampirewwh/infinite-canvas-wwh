export default {
    ...
    agent: {
        ...
        state: { ready: "就绪", connectionRequired: "请填写 Local URL 和 Connect token", invalidUrl: "Local URL 格式不正确", offline: "离线", skillReadFailed: "读取 Skill 失败", skillParseFailed: "Skill 解析失败", requestFailed: "本地 Agent 请求失败", requestTimeout: "本地 Agent 响应超时，请重试" },
        ...
        connect: { pluginTitle: "方式一：在 Codex 中使用插件", pluginText: "在 Codex app 安装 Infinite Canvas 插件后，通过插件启动画布，插件会自动启动本地 Agent 并带上连接信息。", directTitle: "方式二：直接运行 Agent", directText: "不使用 Codex 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", commandCopied: "命令已复制", pluginReminder: "Codex 插件提醒", pluginReminderText: "只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 npx -y @basketikun/canvas-agent@latest 启动本地 Agent 不会安装 MCP。", removePlugin: "移除插件", removeMcp: "移除手动 MCP", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token", installTitle: "本地 Agent 没在运行？", installText: "Windows 用户复制下面这行命令，在 PowerShell 里粘贴并回车：装一次以后，Agent 会在后台自动运行，不再出现命令行黑框，登录开机也会自己启动。", installCommand: "一键安装", installNotice: "脚本只做三件事：写一个隐藏启动脚本、加入开机启动、立刻在后台启动本地 Agent。macOS / Linux 用户请用上面两种方式手动启动。", providerTitle: "Codex 渠道", providerDescription: "从 CC Switch 读取的 Codex 渠道，切换后画布自带的那份 Codex 会改用该渠道并新建一个对话，不影响你平时使用的 Codex。", providerLoading: "正在读取 CC Switch 配置…", providerFailed: "读取 CC Switch 配置失败，请确认已安装并打开过一次 CC Switch。", providerEmpty: "CC Switch 里还没有可用的 Codex 渠道。", providerCurrent: "使用中", providerUse: "切换", providerOfficial: "官方账号登录", providerSwitched: "已切换到「{{name}}」", providerSwitchFailed: "切换渠道失败", providerNotice: "画布使用自己独立的 Codex 配置目录（~/.infinite-canvas/codex-home），切换只改这份配置，不会动你平时用的 ~/.codex；切换前的旧配置会备份到 ~/.infinite-canvas/codex-config-backups。" },
